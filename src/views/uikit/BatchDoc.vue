<script setup>
import useBatch from '@/composables/getlogic/getbatch.js';
...
const { inputbatch } = storeToRefs(batchStore);
...
onBeforeMount(async () => {
    ...
    await getTableFunc();
    console.log('Batch data fetch initiated.', inputbatch.value);
});
...
watch(
    ...
    (newData) => {
        ...
        for (let idx = 0; idx < docs.length; idx++) {
            ...
            console.log('Processing batch item:', inv);
            ...
            flatRows.push({
                ...
                customer: inv?.packaging?.customer || '',
                formula: inv?.packaging?.formula || '',
                rm_code: inv?.packaging?.rm_code || '',
                name: inv?.packaging?.name || '',
                total_weight_kg: inv?.packaging?.total_weight_kg || '',
...
function applyFiltersAndPaging(data) {
    ...
    batches.value = filtered.slice(start, start + lazyParams.value.rows);
}
 ...
 -->
    <div class="card">
        ...
        >
            ...
            <Column field="rm_code" header="RM #" />
            <Column field="name" header="Product Name" />
            <Column field="total_weight_kg" header="Comparison Weight (kg)" />
        </DataTable>
    </div>
</template>
